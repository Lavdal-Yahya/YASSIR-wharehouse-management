...
- [x] P5-04 Sale UI — cart flow: searchable product picker (shows available qty + suggested price), editable unit price per line, qty stepper, running total
- [x] P5-05 Sale UI — payment step: amount paid now, auto remaining, inline customer create/select, customer forced when remaining > 0, clear confirmation message  *(confirmation is its own screen `/sell/:id/confirmation`, not a toast, per design brief §38.5)*
- [x] P5-06 Sales list (scoped by shop for employees) + sale detail with items, payments, statuses  *(sale detail Cancel button surfaces SALE_HAS_ACTIVE_PAYMENTS details.paymentReferences hint until receipts + register-payment deep-links land in the customer-money PR)*
...
- [x] P6-01 Customer account page: totals (purchases / paid / outstanding), unpaid & partially paid sales with balances, payment history  *(migration + tables landed in PR-A; UI is PR-B)*
...
- [x] P6-05 Register Payment UI from customer page, shop-scoped, confirmation with remaining debt
- [x] P6-06 Sale receipt: print-friendly page (white/black, narrow-printer friendly), all spec fields, browser print
- [x] P6-07 Payment receipt: spec fields incl. debt before/after
- [x] P6-08 Reprint old receipts from sale/payment detail; values frozen from snapshots
