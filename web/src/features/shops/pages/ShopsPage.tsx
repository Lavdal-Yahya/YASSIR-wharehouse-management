    ...
    <div>
      ...
      <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-3">
          ...
          />
          <Input
            label={<>{t('shops.form.address')} <span className="text-xs text-slate-400">({t('common.optional')})</span></>}
            {...form.register('address', { setValueAs: (v) => (v === '' ? null : v) })}
          />
          <Input
            label={<>{t('shops.form.phone')} <span className="text-xs text-slate-400">({t('common.optional')})</span></>}
